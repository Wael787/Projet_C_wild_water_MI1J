...
image() {
    local file=$1
    if [ ! -f "$file" ]; then
        echo "Attention: fichier $dirname/csv/$file absent"
        exit 1
    fi
    ...
    if command -v gnuplot >/dev/null 2>&1; then
            ...
            plot "< grep -v '==' '$file' | head -n 11 | tail -n 10" using 2:xtic(1) with boxes title "Volume"
EOF
...
if [ "$action" = "histo" ]; then
    ...
    # traitement sur le C (maxC, srcC, reelC) 
        ...
        # Appel de la fonction de compilation pour $action.c
        compile $action
...
# Action = leaks 
    ...
    csvfile=$dirname/csv/$action_$usine_clean.csv
